...
def custom_get_best_threshold(output, labels):
    preds = output
    precisions, recalls, thresholds = precision_recall_curve(labels, preds)
    f1_scores = 2 * precisions * recalls / (precisions + recalls + 1e-20)
    best_threshold = thresholds[f1_scores.argmax()]
...
def custom_performance_evaluation(output, labels):
    output = torch.sigmoid(torch.from_numpy(output))
    pred_scores = output
    roc_auc = roc_auc_score(labels, pred_scores)
    prec, reca, _ = precision_recall_curve(labels, pred_scores)
    aupr = auc(reca, prec)

    best_threshold = custom_get_best_threshold(output, labels)
    print("Best_threshold: ", best_threshold)
    #pred_labels = output > best_threshold
    pred_labels = output > 0.5
    precision = precision_score(labels, pred_labels)
    accuracy = accuracy_score(labels, pred_labels)
    recall = recall_score(labels, pred_labels)
    f1 = f1_score(labels, pred_labels)
...
def regression_evaluation(output, labels):
    mae = mean_absolute_error(labels, output)
    mse = mean_squared_error(labels, output)
    r2 = r2_score(labels, output)
    return mae, mse, r2
...
class CustomModulatorPPIDataset(InMemoryDataset):
    def __init__(self, datapath, labels = True):
        super(InMemoryDataset, self).__init__()
        print('datapath\t', datapath)
        ...
        if(self.labels):
            self.label_list = df['label'].tolist()
            self.label_list = torch.cuda.FloatTensor(self.label_list)
    ...
    def __getitem__(self, idx):
        ...
        if(self.labels):
            ...
            return molecule_graph, rdkit_descriptors, protein_feats

    def __len__(self):
        if(self.labels):
            return len(self.label_list)
        else:
            return len(self.molecule_index_list)

class PreModulatorPPIDataset(InMemoryDataset):
    def __init__(self, datapath, labels = True):
        super(InMemoryDataset, self).__init__()
        print('datapath\t', datapath)

        #Get non repeated smiles
        df = pd.read_csv(datapath)
        smiles_set = set()
        self.smiles_list = []
        self.lookup_list = []
        ...
        for index, smiles in enumerate(df["SMILES"]):
            ...
            if(smiles in smiles_set):
                look_index = self.smiles_list.index(smiles)
                self.lookup_list.append(look_index)
            else:
                smiles_set.add(smiles)
                self.smiles_list.append(smiles)
                self.lookup_list.append(len(self.smiles_list) - 1)
        self.lookup_list = np.array(self.lookup_list)
        print("Finished smiles lookup")
        #----------------
    ...
    def get_descriptors(self, smi):
        ...
        return num_rings, num_aromatic_carbocycles, num_aromatic_rings, num_aliphatic_rings, num_aromatic_heterocycles, num_heteroatoms, num_saturated_heterocycles, num_saturated_carbocycles, num_saturated_rings, no_count
    ...
    def process_molecule(self):
        ...
        for smiles in self.smiles_list:
            ...
            rdkit_descriptors.append(list(self.get_descriptors(smiles)))
        self.rdkit_descriptors = torch.FloatTensor(rdkit_descriptors)
        #rdkit_mol_objs_list = [AllChem.MolFromSmiles(s) for s in smiles_list]
        preprocessed_rdkit_mol_objs_list = [m if m != None else None for m in rdkit_mol_objs_list]
        preprocessed_smiles_list = [AllChem.MolToSmiles(m) if m != None else None for m in preprocessed_rdkit_mol_objs_list]
        assert len(self.smiles_list) == len(preprocessed_rdkit_mol_objs_list)
        assert len(self.smiles_list) == len(preprocessed_smiles_list)
    ...
    def process_protein(self):
        ### Load pre-trained bert data
        ESM2 = pd.read_csv(f"./data/features/protein_binding_esm2.csv", header=None)
        ESM2 = ESM2.rename(columns={0:"uniprot_id"})
        ESM2.sort_values('uniprot_id', inplace=True)
        pfeature = pd.read_csv('./data/features/protein_binding_phy.csv')
        pfeature.sort_values('uniprot_id', inplace=True)
        assert pfeature.uniprot_id.tolist() == ESM2.uniprot_id.tolist()
    ...
    def __getitem__(self, idx):
        #molecule_idx = self.smiles_list.index(self.molecule_index_list[idx])
        molecule_idx = self.lookup_list[idx]
        molecule_graph = self.molecule_graph_list[molecule_idx]
        rdkit_descriptors = self.rdkit_descriptors[molecule_idx]
...
#Processes molecules features instead of relying on preprocessed features and indexes them for fast inference
class CustomSmilesDataset(InMemoryDataset):
